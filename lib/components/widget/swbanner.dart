...
class _SwBannerState extends State<SwBanner> {
  ...
  adVertising(AdEventType eventType, int index) {
    ReportUtils.adVertising(
        ...
        advertisingId: widget.data[index]['id'],
        adSlotKey: widget.data[index]['advertise_location_code'],
        adSlotName: widget.data[index]['ad_slot_name'],
        adtype: widget.data[index]['ad_type']);
  }
        ...
        : Padding(
            ...
            child: LayoutBuilder(builder: (BuildContext context, BoxConstraints box) {
              return Container(
                ...
                margin: EdgeInsets.only(bottom: ScreenUtil().setWidth(widget.element['is_margin'] == 1 ? 20 : 0)),
                child: Swiper(
                  autoplayDelay: 10000,
                      ...
                      ? SwiperPagination(
                          ...
                          builder: SwiperCustomPagination(builder: (BuildContext context, SwiperPluginConfig config) {
                            return Row(
                              ...
                              children: widget.data.asMap().keys.map<Widget>((e) {
                                return AnimatedContainer(
                                  duration: Duration(milliseconds: 250),
                                  width: ScreenUtil().setWidth(config.activeIndex == e ? 15.5 : 5),
                                  height: ScreenUtil().setWidth(1.5),
                                  margin: EdgeInsets.only(left: ScreenUtil().setWidth(3)),
                                  decoration: BoxDecoration(
                                      color: DefaultStyle.themeColor,
                                      borderRadius: BorderRadius.circular(ScreenUtil().setWidth(0.8))),
                                );
                              }).toList(),
                            );
                          }))
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    return VisibilityDetector(
                        key: Key(
                            '${ReportUtils.getAdType(AdType.elementBanner)['key']}_ELEMENT_BANNER_${widget.data[index]['id']}'),
                        child: GestureDetector(
                          onTap: () {
                            ...
                            if (widget.data[index]['redirect_type'] == 1) {
                              ...
                              CommonUtils.launchURL(widget.data[index]['link_url'].trim());
                              ...
                              CommonUtils.launchURL(widget.data[index]['link_url'].trim() + '?aff=$aff&piliid=$piliid');
                            }
                          },
                          child: Padding(
                            padding: EdgeInsets.only(
                              bottom: ScreenUtil().setWidth(widget.data.length > 1 ? 5 : 0),
                            ),
                            child: ClipRRect(
                              borderRadius: BorderRadius.circular(5.w),
                              child: PlatformAwareNetworkImage(
                                  noVisibilityDetector: true,
                                  url: widget.data == null ? '' : widget.data[index]['resource_url']),
                            ),
                          ),
                        ),
